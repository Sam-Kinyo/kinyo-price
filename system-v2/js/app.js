...
import { closeSheet, closeQuoteSheet, openQuoteSheet, shuffleArray, calcQuotePrice } from './helpers.js';
...
document.addEventListener("click", (e) => {
  ...
  if (e.target.classList.contains("add-quote-btn")) {
    ...
    const cost = livePrice !== null ? livePrice : (item[ck] ?? item.quote50 ?? 0);
  ...
  if (e.target.classList.contains("line-share-btn")) {
    ...
    if (currentQty) {
        ...
        let canSee = false;
        if (state.userLevel >= 1 && (currentQty == 50 || currentQty == 100)) canSee = true;
        if (state.userLevel >= 2 && currentQty == 300) canSee = true;
        if (state.userLevel >= 3 && (currentQty == 500 || currentQty == 1000)) canSee = true;
        
        if(canSee) {
              const lp = calcQuotePrice(item.cost, Number(currentQty), state.userLevel);
              const priceVal = lp !== null ? lp : getPrice(k);
              costText += `${currentQty}個：${priceVal}\n`;
        } else {
              costText += `${currentQty}個：(無權限)\n`;
        }
    } else {
        if (state.userLevel >= 1) {
            const p50 = calcQuotePrice(item.cost, 50, state.userLevel) ?? getPrice('quote50');
            const p100 = calcQuotePrice(item.cost, 100, state.userLevel) ?? getPrice('quote100');
            costText += `50個：${p50}\n100個：${p100}\n`;
        }
        if (state.userLevel >= 2) {
            const p300 = calcQuotePrice(item.cost, 300, state.userLevel) ?? getPrice('quote300');
            costText += `300個：${p300}\n`;
        }
        if (state.userLevel >= 3) {
            const p500 = calcQuotePrice(item.cost, 500, state.userLevel) ?? getPrice('quote500');
            const p1000 = calcQuotePrice(item.cost, 1000, state.userLevel) ?? getPrice('quote1000');
            costText += `500個：${p500}\n1000個：${p1000}\n`;
        }
    }
...
if (multiLineBtn) {
    multiLineBtn.onclick = () => {
        ...
        Array.from(checked).forEach((c, i) => {
            ...
            if (state.userLevel >= 1 && (qty == 50 || qty == 100)) canSeeQuote = true;
            if (state.userLevel >= 2 && qty == 300) canSeeQuote = true;
            if (state.userLevel >= 3 && (qty == 500 || qty == 1000)) canSeeQuote = true;

            const liveP = calcQuotePrice(item.cost, Number(qty), state.userLevel);
            const cost = canSeeQuote ? (liveP !== null ? liveP : (item[ck] ?? item.quote50 ?? "-")) : "---";
