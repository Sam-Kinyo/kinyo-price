...
/* 即時報價計算 (依 userLevel 動態計算) */
const QUOTE_DIVISORS = {
  // Level 3: 可看 50/100/300/500/1000
  3: { 50: 0.75, 100: 0.78, 300: 0.81, 500: 0.835, 1000: 0.858 },
  // Level 2: 可看 50/100/300
...
/**
 * 即時計算報價 (無條件進位)
 * @param {number} cost - 廠價(成本)
 * @param {number} qty  - 起訂量 tier (50/100/300/500/1000)
 * @param {number} level - userLevel (1~4)
 * @returns {number|null} 報價金額，若無法計算回傳 null
 */
export function calcQuotePrice(cost, qty, level) {
  if (!cost || cost <= 0) return null;
  // Level 4 使用 Level 3 的公式
  const effectiveLevel = level >= 3 ? 3 : level;
  const tierMap = QUOTE_DIVISORS[effectiveLevel];
  if (!tierMap) return null;
  const divisor = tierMap[qty];
