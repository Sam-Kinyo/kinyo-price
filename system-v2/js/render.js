...
export function renderResults(list) {
  ...
  list.forEach(item => {
    ...
    const getLivePrice = (qty) => {
      const v = calcQuotePrice(item.cost, qty, state.userLevel);
      return v !== null ? v : (item['quote' + qty] ?? "-");
    };
    ...
    if (state.isGroupBuyUser) {
        ...
        if (state.userLevel >= 1) {
            priceHtml += `<span class="price-tag tag50">50個：${getLivePrice(50)}</span>`;
            priceHtml += `<span class="price-tag tag100">100個：${getLivePrice(100)}</span>`;
        }
        if (state.userLevel >= 2) {
            priceHtml += `<span class="price-tag tag300">300個：${getLivePrice(300)}</span>`;
        }
        if (state.userLevel >= 3) {
            priceHtml += `<span class="price-tag tag500">500個：${getLivePrice(500)}</span>`;
            priceHtml += `<span class="price-tag tag1000">1000個：${getLivePrice(1000)}</span>`;
        }
    }
      ...
      tiers.forEach(t => {
          ...
          if (isVisible) {
              quoteHtml += `<div style="display:flex; justify-content:space-between; padding:4px 0; border-bottom:1px dashed #eee;">
                  ...
                  <span style="font-weight:bold; color:#2563eb;">${getLivePrice2(t)}</span>
              </div>`;
          }
      });
  }
      ...
      tiers.forEach(t => {
          ...
          if (isVisible) {
              quoteHtml += `<div style="display:flex; justify-content:space-between; padding:2px 0; border-bottom:1px dashed #eee;">
                  <span style="color:#666;">${t}個</span>
                  <span style="font-weight:bold; color:#2563eb;">${getLivePrice3(t)}</span>
              </div>`;
          }
      });
  }
