...
import { fetchAsDataURL, getInventoryRangeLabel, calcQuotePrice } from './helpers.js';
...
async function buildProductSlide(pptx, item, tier, customQuoteInfo) {
    ...
    if (customQuoteInfo) {
        ...
        const liveP = calcQuotePrice(item.cost, Number(tier), state.userLevel);
        const p = liveP !== null ? liveP : (item[quoteKey] || item.quote50 || "-");
