    ...
    reader.onload = (evt) => {
      try {
        ...
        jsonData.forEach((row) => {
          ...
          if (tempCost > 0) {
            obj.cost = tempCost;
            obj.price_75  = Math.round((tempCost / 0.75)  * 1.05);
            obj.price_78  = Math.round((tempCost / 0.78)  * 1.05);
            obj.price_81  = Math.round((tempCost / 0.81)  * 1.05);
            obj.price_835 = Math.round((tempCost / 0.835) * 1.05);
            obj.price_858 = Math.round((tempCost / 0.858) * 1.05);

            if (!hasVal(row[h.q50]))   obj.quote50   = Math.ceil((tempCost / 0.75) * 1.05);
            else obj.quote50 = row[h.q50]; 
            if (!hasVal(row[h.q100]))  obj.quote100  = Math.ceil((tempCost / 0.78) * 1.05);
            else obj.quote100 = row[h.q100];
            if (!hasVal(row[h.q300]))  obj.quote300  = Math.ceil((tempCost / 0.81) * 1.05);
            else obj.quote300 = row[h.q300];
            if (!hasVal(row[h.q500]))  obj.quote500  = Math.ceil((tempCost / 0.835) * 1.05);
            else obj.quote500 = row[h.q500];
            if (!hasVal(row[h.q1000])) obj.quote1000 = Math.ceil((tempCost / 0.858) * 1.05);
